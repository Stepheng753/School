...
class Modulo extends Token
{
    ...
    Modulo (Token leftNum, Token rightNum){
        super(leftNum, rightNum);
    }

    // Implement the inherited abstract method evaluate.
    // Evaluate should return the String representation of the result of finding the modulus of the
    ...
    String evaluate() {
        try {
            int modulusTokens = Integer.parseInt(leftNum.evaluate()) % Integer.parseInt(rightNum.evaluate());
            return Integer.toString(modulusTokens);
        }
